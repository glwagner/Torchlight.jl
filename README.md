...
| full Float32 | **not accepted** | Adam first-step update fails the frozen Float32 tolerance on 18 of 262,144 components of one block; diagnosed as Float32 ill-conditioning of `g/(|g|+ε)` for `|g| ≲ ε`, not an optimizer-rule mismatch (the Float64 fixture passes the same check). Retained as a finding, not loosened. |
