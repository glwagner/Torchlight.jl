...
Purpose (sharpened in this revision): the networks of interest are
**parameterizations for Earth system models** — machine-learned closures for
radiation, convection, turbulence, surface fluxes, sea ice and ocean processes
— developed in PyTorch or JAX and needed inside differentiable Julia models.
...
| Build on Luximm's infrastructure; Luximm fixture reader forces Float32 | **Changed.** Luximm was first used as a pinned git dependency for one leaf-setting function, then dropped: its purpose (timm vision backbones) differs from ours, it is unregistered, and it pulls in an unused model zoo. Torchlight now has no unregistered dependencies. Luximm's fixture-first porting *workflow* is acknowledged as the inspiration (§11). `read_fixture` preserves dtype; `map_parameters` adds coverage/duplicate/shape/dtype checks and evaluates each transform once. |
...
| M0 "reproduce one existing Luximm vision variant's parity" | **Dropped** (§9). Running a timm backbone parity adds no evidence about Earth-system parameterizations; the fixture-layout and mapping-triple conventions were adopted directly. |
...
**Executed.** Schema v1 is documented in `docs/src/fixture_schema.md`. Around
the conventional `/input`, `/state_dict/<key>`, `/output` core it adds `/meta`
...
Dependencies: Lux, Reactant, Enzyme, Zygote, Optimisers, HDF5 — all
registered. No source is adapted from other projects.

## 9. Milestones, effort, and exit criteria

| Milestone | Draft budget | Outcome |
|---|---|---|
| M0 environment, contract, conventions | 0.5–1 day | **Done as re-scoped.** Tested versions recorded and locked (`Manifest.toml` committed; `python/requirements-lock.txt`); HDF5/dtype/axis conventions verified; Reactant+Enzyme probe passed; tiny MLP mapping verified. The Luximm vision-variant control was dropped from scope (§0). |
...
Status against exit criteria: M0, M1 and M2 met within one session versus the
≤ 1 week budget; M3 partial as tabulated. No backend was removed. The
...
the compatibility declaration); registered dependencies only; fixture schema v1; PyTorch-first
...
Open: Float32 Adam acceptance policy (§7); graph-import experiment scope;
whether to coordinate a generic loader with Lux PR #1658. No upstream issue, comment, or PR has been created.
...
| Luximm.jl `8e26bbe` (30 Aug 2026) | timm vision backbones ported to Lux; Julia 1.12, Lux ^1.31.4 | Related work: its porting guide inspired the fixture-first workflow. Not a dependency; no code shared. |
...
## 12. Relationship to Luximm

The draft proposed building on Luximm's infrastructure. In execution the
reusable part turned out to be its *workflow* — capture reference fixtures
first, implement an understandable native model, map every state-dict key,
compare end to end and then stage by stage — and its fixture layout and
mapping-triple conventions, which Torchlight adopts. Its code targets timm
vision backbones, casts to Float32, and is unregistered, so Torchlight owns its
reader, transforms and leaf-setter and has no dependency on Luximm. The
implementation sequence (2) tiny asymmetric MLP, (3) Float64 + gradients,
(4) paper-size MLP + training was executed; the vision-parity control (1) was
dropped from scope.
...
4. Second architecture (convolution or normalization) to stress mapping and
   ...
   scientific case.
5. Extend the case list toward the parameterizations of interest: surface
   fluxes, convection, ocean/sea-ice closures, with their host-model coupling
   tested through a small differentiable integrator.
